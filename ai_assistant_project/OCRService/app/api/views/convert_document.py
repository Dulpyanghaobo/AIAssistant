...
@router.post("/")
async def convert_document(document: UploadFile = File(...), ocr_service: OCRService = Depends(get_ocr_service)):
    contents = ocr_service.process_input(input_data=document, processor_type="text")
    result = ocr_service.export(data=contents, exporter_type="document_converter")
    return result
